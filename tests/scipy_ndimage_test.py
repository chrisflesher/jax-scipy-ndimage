...
from jax_scipy_ndimage import ndimage as lsp_ndimage
...
class NdimageTest(jtu.JaxTestCase):
  ...
  def testMapCoordinates(self, shape, dtype, coords_shape, coords_dtype, order,
    ...
    with jtu.strict_promotion_if_dtypes_match([dtype, int if round else coords_dtype]):
      if dtype in float_dtypes:
        epsilon = max(jax.dtypes.finfo(jax.dtypes.canonicalize_dtype(d)).eps
  ...
  def testContinuousGradients(self):
    ...
    self.assertAllClose(jax.grad(loss)(0.5), 1.0, check_dtypes=False)
    self.assertAllClose(jax.grad(loss)(1.0), 2.0, check_dtypes=False)
